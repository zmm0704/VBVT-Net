...
parser.add_argument(
    "--logdir",
    type=str,
    default="model/hologic_sim_model_our_3/net_latest.pth",
    help="path to model and log files",
)
parser.add_argument(
    "--save_path",
    type=str,
    default="results/hologic_sim_model_our_3/",
...
def main():
    ...
    loader_val = DataLoader(
        dataset=dataset_val, num_workers=4, batch_size=1, shuffle=False
    )
    ...
    for num, item in enumerate(loader_val, 0):
        ...
        if opt.use_GPU:
            save_out = de_normalize_NF(
                np.float32(output[0].data.cpu().numpy().squeeze())
            )  # back to cpu
